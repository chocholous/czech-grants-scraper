...
class ProxyConfiguration extends core_1.ProxyConfiguration {
    ...
    constructor(options = {}, config = configuration_1.Configuration.getGlobalConfig()) {
        ...
        super({ proxyUrls, newUrlFunction, ['validateRequired']: false });
        ...
        const { groups = [], apifyProxyGroups = [], countryCode, apifyProxyCountry, password = config.get('proxyPassword'), } = options;
    ...
    async _setPasswordIfToken() {
        ...
        if (token) {
            const { proxy } = await actor_1.Actor.apifyClient.user().get();
    ...
    async _fetchStatus() {
        ...
        for (let attempt = 1; attempt <= CHECK_ACCESS_MAX_ATTEMPTS; attempt++) {
            try {
                const response = await (0, got_scraping_1.gotScraping)(requestOpts);
