...
Before you start, read the PRD.md file to understand the project goals and requirements.
...
- The Actor should be able to scrape the data from the sources.
- The Actor should be able to store the data in a shared dataset.
- The Actor should be able to serve search queries over the stored dataset.
- The Actor should be able to refresh stale data on a weekly cadence.

## Project Structure & Module Organization
- `scrapers/` holds the main Python scrapers. `scrapers/grants/` is the active area with the `dotaceeu.py` orchestrator and `sources/` for individual sources.
- `tests/` contains pytest tests (currently `tests/test_scrapers.py`).
- `utils/` includes shared helpers used across scrapers.
- `docs/` contains contributor docs and research notes.
...
- New scrapers should live in `scrapers/grants/sources/` and be wired into the registry.
- Keep configuration in `scrapers/grants/config.yml` and environment secrets in `.env`.

## Testing Guidelines
- Framework: pytest (see `tests/test_scrapers.py`).
- Name tests `test_*.py` and test functions `test_*`.
- Add a basic import or model test for each new source and any parsing edge cases.

## Commit & Pull Request Guidelines
- Use short, imperative messages (e.g., "Add OP TAK parser").
- For PRs, include a concise description, link any related issues, and mention how you validated (commands run, sample outputs). If output changes, attach a small before/after sample from `data/`.
