"""
Sub-scraper for army.cz (Ministry of Defence of the Czech Republic).
"""
...
class ArmyCzScraper(AbstractGrantSubScraper):
    """Scraper for army.cz grant calls"""

    DOMAIN = "army.cz"

    def can_handle(self, url: str) -> bool:
        """Check if URL is from army.cz domain"""
        parsed = urlparse(url)
        return self.DOMAIN in parsed.netloc

    async def extract_content(self, url: str, grant_metadata: dict) -> Optional[GrantContent]:
        """Extract content from army.cz grant page"""
        try:
            response = requests.get(url, timeout=10)
            response.raise_for_status()
            soup = BeautifulSoup(response.content, 'html.parser')

            # Army.cz often has content in .content-inner or similar
            content_elem = soup.select_one('.content-inner') or soup.select_one('#main-content') or soup.select_one('article')
